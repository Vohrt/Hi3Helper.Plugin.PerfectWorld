...
public partial class PerfectWorldGameInstaller
{
    ...
    private async Task<ManifestBundle> GetManifestBundleAsync(CancellationToken token, bool forceRefresh = false)
    {
        ...
        try
        {
            ...
            // 异环/NTE this removes every per-language voice pack under Content/TagPatchPaks/: the plugin ships only the
            // base game (DeferredContentKeepMarkers is empty, so no voice is kept) and the in-game updater downloads the
            // voice matching the current system language on demand. Filtering here keeps the size query, install and
            // update paths mutually consistent because all of them read the manifest exclusively through this (cached)
            // method.
    ...
    ///     optional <see cref="PerfectWorldGameConfig.DeferredContentKeepMarkers"/> exceptions: for 异环/NTE it drops
    ///     every per-language voice pack under <c>Content/TagPatchPaks/</c> (no keep exceptions), leaving the in-game
    ///     updater to download the voice matching the current system language. A directly-addressed <c>&lt;Res&gt;</c>
    ///     file is dropped when its path is deferred (matches a marker and no keep exception); a packed
    ///     <c>&lt;Pak&gt;</c> archive is dropped only when <em>every</em> file it bundles is deferred, because a pak
    ...
    ///     state records the installed base build together with any voice/tag language sections the plugin kept on disk
    ///     (none for NTE, which defers every voice), so the in-game client no longer sees a zero local version (which
    ///     caused the "更新失败" loop) and downloads the language it selects on demand. The flat <c>tmp/client.xml</c> is
