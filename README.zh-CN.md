...
* **异环**（UE5，`HTGame.exe`）。启动器取自其自更新清单（`.../publish_ob/launcher/Version.ini` → `AllFiles.xml`），
  ...
  会运行该进程内更新器，插件只安装本体加一种默认（中文）语音，并把其余语言**延迟**到游戏内按需下载（详见下文「按需语音」）。
...
**按需语音（异环）——只安装默认语音，其余在游戏内下载。** 官方启动器只安装本体加一种默认（中文）语音，
其余语言（日 / 英 / 韩）留给游戏内的更新器按需下载，因此官方全新安装会把这三种语言显示为带下载大小。插件现在
复刻了这一做法：它安装本体加默认中文语音（`pakchunk101`，通过 `DeferredContentKeepMarkers` 保留），并把其余
语音包（`pakchunk102/103/104` = 日 / 英 / 韩，位于 `Content/TagPatchPaks/`，由 `DeferredContentPathMarkers` 匹配）
**延迟**下载，因此异环的安装体积比一次性打包全部语言约小 5 GB。之所以可行，是因为**默认的自动点击启动路径**（见
下文）会让 `NTEGame.exe` 运行其正常的进程内资源/语音对账——正是 `/autoplay` 会跳过的那一步——从而按需下载每种被
延迟的语言。为避免该对账陷入「更新失败」死循环，插件会在每次安装/更新后伪造原生 `PatcherSDK` 状态文件（`config.xml`
/ `ResList.xml` / `tmp\client.xml`，即 `WritePatcherState = true`），把本体加默认语音记录为已安装、把被延迟的语言
记录为尚未安装；否则补丁器会看到本地版本 0.0，认为整个版本缺失而陷入死循环。（早期版本曾把这种延迟与去掉
