...
启动器会以**最小化**方式启动（缩到任务栏），因此在这段短暂的自动登录启动过程中不会弹到前台；在默认的自动点击
路径下，一旦「开始游戏」按钮被按下，启动器便会自行最小化到托盘，并随游戏一同退出。
...
* **回退阶梯（绝不会让你卡住）。** 若自动点击无法启用（Collapse 非管理员，或嵌入的 DLL 无法准备），插件改用旧的
  `/autoplay` 路径。若*尝试*注入但失败，则启动器以**不带** `/autoplay` 的方式启动（最小化到任务栏），好让你自己
  还原窗口并点击「开始游戏」。若注入成功但点击始终未触发，启动器会停留在按钮界面——从任务栏还原它并手动点击即可。
